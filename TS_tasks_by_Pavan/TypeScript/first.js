...
function filterByCity(persons, query) {
    ...
    return persons.filter(person => person.city.toLowerCase() === lowerQuery || person.name.toLowerCase() === lowerQuery);
}
// DOM elements
const cityInput = document.getElementById('cityInput'); // Input field for city name
const filterButton = document.getElementById('filterButton'); // Button to trigger filtering
const results = document.getElementById('results'); // List to display filter results
